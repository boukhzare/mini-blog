...
<div class="max-w-2xl mx-auto">
    ...
    <h1 class="text-xl font-semibold text-gray-800 mb-6">Edit Post</h1>
    ...
    <form method="POST" action="{{ route('posts.update', $post->id) }}"
        ...
        <div>
            ...
            <textarea name="content" rows="4"
                      class="mt-1 w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-300">{{ old('content', $post->content) }}</textarea>
            @error('content')
                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
            @enderror
        </div>
